...
$SQL = "(SELECT * FROM dailyfunds WHERE type = 'Philequity Peso Bond Fund, Inc'
	...
	AND date_navps <= DATE_ADD('$latest_date_navps', INTERVAL -1 MONTH) AND status = '1' ORDER BY date_navps DESC LIMIT 1 )
	";
$SQL_res = mysql_query($SQL);
for($n=0;$n<2;$n++){
	$SQL_rec = mysql_fetch_array($SQL_res); 
...
$pefi_beg = round(((($mo1new/1.3216) - 1) * 100),2) ;
...
$psei_beg = round(((($mo1new/1478.68) - 1) * 100),2);
...
$SQL = "(SELECT * FROM dailyfunds WHERE type = 'Philequity Peso Bond Fund, Inc'
	...
	UNION
	(SELECT * FROM dailyfunds WHERE type = 'Philequity Peso Bond Fund, Inc'
	AND date_navps <= DATE_ADD('$latest_date_navps', INTERVAL -6 MONTH) AND status = '1' ORDER BY date_navps DESC LIMIT 1 )
	";
$SQL_res = mysql_query($SQL);
for($n=0;$n<2;$n++){
	$SQL_rec = mysql_fetch_array($SQL_res); 
...
$SQL = "(SELECT * FROM dailyfunds WHERE type = 'Philequity Peso Bond Fund, Inc'
	...
	UNION
	(SELECT * FROM dailyfunds WHERE type = 'Philequity Peso Bond Fund, Inc'
	AND date_navps <= DATE_ADD('$latest_date_navps', INTERVAL -1 YEAR) AND status = '1' ORDER BY date_navps DESC LIMIT 1 )
	";
$SQL_res = mysql_query($SQL);
for($n=0;$n<2;$n++){
	$SQL_rec = mysql_fetch_array($SQL_res); 
...
$comma = "";
$pcom = "";
$query = "SELECT date_format(date_navps,'%Y') as year FROM `dailyfunds` WHERE `type` = 'Philequity Peso Bond Fund, Inc' AND status = '1'
...
for($i=0;$i<$row;$i++){
	...
	// PEFI DATA
	$qry = "(SELECT * FROM dailyfunds WHERE type = 'Philequity Peso Bond Fund, Inc' 
						...
						<table class="table table-bordered table-striped">
						  <thead>
							<tr>
							 ...
							 <th class="text-center " colspan="1">PBBF</th>
						  ...
						  <tbody>
						  	...
						  	<tr class="edit" id="detail">
								...
								<td class="text-center"><?php echo $pefi_return[0] ?></td>
					  ...
					  <div class="panel-body table-responsive">
						<table class="table table-bordered table-striped">
						  <thead>
							...
							<!-- <tr>
							  <th class="text-center"></th>
							 <th class="text-center " colspan="1">PPBF</th>
						  ...
						  <tbody>
							<tr class="edit" id="detail">
							  ...
							  <td id="name" class="text-center"><?php echo $pefi_return[0] ?></td>
							...
							<tr>
							  ...
							  <td class="text-center"> <?php echo $pefi_return[1] ?> </td>
							...
							<tr>
							  ...
							  <td class="text-center"> <?php echo $pefi_return[2] ?> </td>
							
							</tr>

							<tr>
							  <td class="text-center"> <?php echo $yr_return[3] ?> </td>
							  <td class="text-center"> <?php echo $pefi_return[3] ?></td>
							...
							<tr>
							  ...
							  <td class="text-center"> <?php echo $pefi_return[4] ?> </td>
							...
							<tr>
							  ...
							  <td class="text-center"> <?php echo $pefi_return[5] ?> </td>
					    ...
					    <div class="panel-footer">
							...
							<a href="<?php echo site_url('/download') ?>" target="_blank" class="theme-btn"><span class="glyphicon icon-share"></span>Download Prospectus</a>
