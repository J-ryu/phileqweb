...
$SQL = "SELECT DATE_FORMAT(date_navps,'%Y') as year FROM dailyfunds WHERE type = 'Philequity Dividend Yield Fund, Inc' AND status = '1' ORDER BY date_navps ASC LIMIT 1 ";
$SQL_res = mysql_query($SQL);
$SQL_rec = mysql_fetch_array($SQL_res); 
$startpefi = $SQL_rec["year"]; 

$SQL = "SELECT * FROM dailyfunds 
	WHERE type = 'Philequity Dividend Yield Fund, Inc' AND 
...
$psei_beg = round(((($mo1new/6011.14 ) - 1) * 100),2);
...
$comma = "";
$pcom = "";
$query = "SELECT date_format(date_navps,'%Y') as year FROM `dailyfunds` WHERE `type` = 'Philequity Dividend Yield Fund, Inc' AND status = '1'
...
for($i=0;$i<$row;$i++){
	...
	// PEFI DATA
	$qry = "(SELECT * FROM dailyfunds WHERE type = 'Philequity Dividend Yield Fund, Inc' 
	...
	$pefireturn1.= $pcom.round(((($pefinew/0.9630) - 1 ) * 100),2);
	$pcom = ',';
						...
						<table class="table table-bordered table-striped">
						  <thead>
							<tr>
							 ...
							 <th class="text-center " colspan="1">PDYF</th>
